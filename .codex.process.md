...
- When a user prompt references changes across multiple files, perform all changes during one editing run.
...
1. **Create and switch to a new branch**
   ...
   - Create a new branch unless explicitly asked to reuse.

2. **Perform all edits**
   - Modify each file needed to fulfill the request.
   - Maintain consistent formatting, naming, spacing, and architecture patterns.
   - Prefer consistency with existing project structure unless the prompt requests a restructure.
...
- Prefer small helpers and cohesive patterns over large rewrites.
- Use existing naming conventions and avoid introducing new patterns unless necessary.

---

## 4. File Creation & Organization
- Only create new files if it clearly improves clarity/maintainability or the prompt requires it.
- Place new files in logical locations consistent with the repo (e.g., `assets/`, `css/`, `js/`, `src/`, `components/`, etc. if they exist).
- When adding styling, reuse existing conventions (e.g., existing CSS organization, naming, and layout approach).
- Keep mobile-friendly behavior in mind when working on UI and layout.

---

## 5. Safety & Error Handling
- If a prompt requests something that contradicts existing project constraints or introduces obvious risk (security, data loss), warn the user once before proceeding.
...
Unless a prompt explicitly instructs: **“do not commit,” “do not push,” or “no PR summary,”** Codex must automatically perform the full git workflow.
...
### 6.3 Pull Request Summary Output
...
Before committing and pushing, Codex must verify as appropriate for the repo:

- No syntax errors
- No unused/dead code introduced unnecessarily
- No stray debug code
- No accidental changes outside the scope of the prompt
- If the repo includes scripts (e.g., npm), run or logically validate the relevant commands (e.g., `npm run lint`, `npm test`, `npm run build`)
- If the repo is static HTML/CSS/JS, validate by:
  - Ensuring the site loads without console errors
  - Ensuring key UI flows still work
  - Checking mobile responsiveness at common breakpoints

If Codex cannot validate, it must notify the user and explain what it could not verify.

---

## 8. Documentation
- Update relevant documentation when new features introduce new usage or setup steps.
